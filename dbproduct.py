...
class product:
    ...
    def __init__(self, window):
        ...
        # Buttons
        ttk.Button(text = 'DELETE', command = self.delete_product).grid(row = 5, column = 0, sticky = W+E, columnspan = 1)
        ttk.Button(text = 'EDIT', command = self.edit_product).grid(row = 5, column = 1, sticky = W+E,  columnspan = 2)
...
# Function to Execute Database Querys
    ...
    def add_product(self):
        if self.validation():
            ...
            self.message['text'] = 'product {} added Successfully'.format(self.product.get())
    ...
    def edit_records(self, new_product, old_product, new_description, old_description):
        query = 'UPDATE product SET product = ?, description = ? WHERE product = ? AND description = ?'

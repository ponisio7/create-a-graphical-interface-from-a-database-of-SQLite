...
for a in (information_from_the_database.get(table_names[0]).get('columns')):
...
class Product:
    ...
    stringpy+="##'.format(table_names[0])\n        self.wind.title(\'" + '{}'.format(table_names[0])+"\') \n\n        "
    stringpy+="# Creating a Frame Container\n        ##'.format(table_names[0])\n        "
    stringpy+="frame = LabelFrame(self.wind, text = \'Register new "+ '{}'.format(table_names[0])+"\')\n        "
    ...
    for x in column_list:
        ...
        count_+=1   
    count_row=count_row+1
    stringpy+="# Button Add\n        ttk.Button(frame, text = 'Save "+ '{}'.format(table_names[0])+"\'"
    stringpy+=", command = self.add_"+ '{}'.format(table_names[0])+").grid(row = " + '{}'.format(str(count_row)) + ", columnspan = 2, sticky = W + E)"
    stringpy+="\n\n        # Output Messages \n        self.message = Label(text = '', fg = 'red')\n        "
    stringpy+="self.message.grid(row = " +'{}'.format(str(count_row)) +", column = 0, columnspan = 2, sticky = W + E)\n\n        # Creating a Frame Container\n        "
    stringpy+="container = LabelFrame(self.wind, text = 'Table of " + '{}'.format(table_names[0])+"\')\n        container.grid(row = "
    count_row=count_row+1
    ...
    count_row=count_row+1
    stringpy+="ttk.Button(text = 'DELETE', command = self.delete_"+ '{}'.format(table_names[0])+").grid(row = "+'{}'.format(str(count_row))+", column = 0, sticky = W+E, columnspan = 1)"
    stringpy+="\n        ttk.Button(text = 'EDIT', command = self.edit_"+ '{}'.format(table_names[0])+").grid(row = "+'{}'.format(str(count_row))+", column = 1, sticky = W+E,  columnspan = 2)"
    stringpy+="\n\n        # Filling the Rows\n        self.get_"+ '{}'.format(table_names[0])+"s()\n\n    "
    stringpy+="# Function to Execute Database Querys\n    def run_query(self, query, parameters = ()):\n        with sqlite3.connect(self.db_name) as conn:"
    stringpy+="\n            cursor = conn.cursor()\n            result = cursor.execute(query, parameters)\n            conn.commit()"
    stringpy+="\n        return result\n\n    # Get "+ '{}'.format(table_names[0])+" from Database\n    def get_"+ '{}'.format(table_names[0])+"s(self):"
    stringpy+="\n        # cleaning Table\n        records = self.tree.get_children()\n        for element in records:\n            self.tree.delete(element)"
    stringpy+="\n        # getting data\n        query = \'SELECT * FROM " + '{}'.format(table_names[0])+ " ORDER BY " + '{}'.format(column_list[0])
    ...
    for name_ in column_list:
        ...
        count_+=1
    count_=0
    stringpy+="def add_"+ '{}'.format(table_names[0])+"(self):\n        if self.validation():\n            query = \'INSERT INTO "+ '{}'.format(table_names[0])+" VALUES(NULL, "
    for number in range(len(column_list)-1):
        count_+=1
        if(count_!= len(column_list)-1):
            stringpy+="?, "
    ...
    for name_ in column_list:
        ...
        count_+=1
    stringpy+='{}'.format(table_names[0])+" {} added Successfully'.format(self."+ '{}'.format(column_list[1])+".get())\n            "
    count_=0
    for name_ in column_list:
        if (count_!=0 and count_<len(column_list)-1):
    ...
    for name_ in column_list:
        ...
        count_+=1
    stringpy+="self.edit_wind = Toplevel()\n        self.edit_wind.title = \'Edit "+ '{}'.format(table_names[0])+"\'\n        "
    count_=0
    rows=1
    for name_ in column_list:
        if (count_!=0 ):
    ...
    for name_ in column_list:
        if (count_!=0 and count_<len(column_list)-1):
            ...
            if(count_== len(column_list)-1):
                stringpy+="new_"+name_+", old_"+name_+"):\n        query = 'UPDATE " + '{}'.format(table_names[0]) + " SET "
        count_+=1
    count_=0
    for name_ in column_list:
        if (count_!=0 and count_<len(column_list)-1):
    ...
    stringpy+="\n\nif __name__ == '__main__':\n    window = Tk()\n    application = "+table_names[0]+"(window)\n    window.mainloop()"
        #print(stringpy)
